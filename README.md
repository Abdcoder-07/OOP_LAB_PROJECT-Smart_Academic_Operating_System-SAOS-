...
Select Console Application: Choose C++ Project, name it SAOSPortal, and click OK.
Add All Files: Right-click the project in the sidebar → Add to Project → select all .cpp and .h files.
Configure Compiler: Go to Tools > Compiler Options. Check "Add the following commands when calling the compiler" and enter:
-std=c++11
Build & Run: Press F11 to compile and launch the system.
Option B — VS Code
Prerequisite: Install the C/C++ and C++ Runner extensions from the VS Code marketplace.

Open Folder: Open the project root directory in VS Code (File > Open Folder).
Open Terminal: Press Ctrl+` to open the integrated terminal.
Compile using the following single command (ensure all files are in the same directory):
